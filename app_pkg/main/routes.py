...
from app_pkg.main.forms import EditProfileForm, EmptyForm, PostForm, SearchForm
...
def index():
    ...
    return render_template('index.html', title=_('Home'), posts=posts.items, form=form, next_url=next_url,
                           prev_url=prev_url)
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    # Hide link to user in user profile page by providing condition in _post.html page.
...
def search():
    ...
    # Need to add user names and their posts along with it. Also add to the total.
    # posts, total = User.search(g.search_form.q.data, page, current_app.config['POSTS_PER_PAGE'])
