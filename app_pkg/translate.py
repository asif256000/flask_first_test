...
def translate(text_to_translate, source_language, target_language):
    ...
    if "GOOGLE_TRANSLATOR_KEY" in current_app.config and current_app.config["GOOGLE_TRANSLATOR_KEY"]:
        g_res = google_translate(text_to_translate, source_language, target_language)
        if g_res["status_code"] == 200:
            return g_res["translated_text"]
    if ("MS_TRANSLATOR_KEY" not in current_app.config or not current_app.config["MS_TRANSLATOR_KEY"]) and (
...
def google_translate(text, source_lang, dest_lang):
    ...
    r = requests.post("https://translation.googleapis.com/language/translate/v2", params=auth, json=payload)
